...
"""
 ...
 Fast, content based duplicate file hunter with cache and more!
 Copyright ©2018-2021 Marcin Orlowski <mail [@] MarcinOrlowski.com>
...
class Log(object):
    ...
    @classmethod
    def configure(cls, config: ConfigBase):
    ...
    @staticmethod
    def banner(messages, ignore_quiet_switch: bool = False, add_to_history: bool = True,
        ...
        Log._log(messages, None,
                 ignore_quiet_switch, add_to_history)

    @staticmethod
    def banner_v(messages, ignore_quiet_switch: bool = False, add_to_history: bool = True,
                 top: bool = True, bottom: bool = True):
    ...
    # warning
    @staticmethod
    def w(message=None, condition=True):
        if condition and message is not None:
            messages = Log._to_list(message)
            _ = [Log._log('**WARN** ' + Log.strip_ansi(msg), Log.COLOR_WARN, True) for msg in messages]
